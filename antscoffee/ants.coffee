...
class Game
	...
	class Ant extends Location
		constructor: (@game, data, @type=LAND_TYPES.ANT, @is_alive=yes) ->
			[@x, @y, @owner] = data

		order: (direction) => @game.issue_order(@x, @y, direction)
		neighbor: (direction) => @game.neighbor(@x, @y, direction)
		canMove: (direction) =>
			point = @neighbor(direction)
			@game.passable(point.x, point.y)

	class Hill extends Location
		constructor: (@game, data, @type=LAND_TYPES.HILL) ->
			[@x, @y, @owner] = data
	...
	parse: (line) =>
		...
		if command in CONFIG_COMMANDS
			...
			switch command
				...
				when "h"
					@MAP[x][y] = new Hill this, data
				when "a"
					@MAP[x][y] = new Ant this, data
				when "d"
					@MAP[x][y] = new Ant this, data, is_alive=no
		return command
